		...
		<div class="row">
			<div class="sl-content-wrap clearfix">
				<div class="col-sm-12 col-md-9 sl-main-content">
					...
					<?php } ?>
					<form action="{:U('Edit/'.$topic['id'].'')}" method="post" id="question_form">
						<div class="sl-mod sl-mod-publish">
							<div class="mod-body">
								<h4>标题:</h4>
								<div class="sl-publish-title active">
									<div>
										<input type="text" placeholder="标题为3-50个字符" name="title" id="question_contents" value="{$topic.title}" class="form-control" />
									</div>
								</div>
								<h4>帖子内容:</h4>
								...
								<div class="sl-mod sl-editor-box">
									<div class="mod-head">
										<div class="wmd-panel">
								            <textarea class="wmd-input form-control autosize editor" id="content" rows="8" name="content"  placeholder="内容至少5个字符">{$topic.content}</textarea>
								        </div>
									</div>
									<h4>选择版块:</h4>
								<div class="sl-publish-title active">
									<div>
										<select name="forum" class="form-control">
											<volist name="forum_list" id='forum'>
												<option value="{$forum.id}" <IF condition="$forum['id'] eq $topic['forum_id']">selected = "selected"</IF>>{$forum.name}</option>
											</volist>
										</select>
									</div>
								</div>
