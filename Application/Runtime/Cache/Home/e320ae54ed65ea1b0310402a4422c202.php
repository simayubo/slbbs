...
<div class="sl-container-wrap">
 	<div class="nav-title"><a href="/">首页</a>》编辑帖子</div>
			...
			<div class="sl-content-wrap clearfix">
				<div class="col-sm-12 col-md-9 sl-main-content">
					<ul class="nav nav-tabs sl-nav-tabs active">
						<h2 class="hidden-xs"><i class="icon icon-ask"></i> 编辑帖子</h2>
					</ul>
					<?php if(!empty($errmsg)){ ?>
					...
					<?php } ?>
					<form action="<?php echo U('Edit/'.$topic['id'].'');?>" method="post" id="question_form">
						<div class="sl-mod sl-mod-publish">
							<div class="mod-body">
								<h4>标题:</h4>
								<div class="sl-publish-title active">
									<div>
										<input type="text" placeholder="标题为3-50个字符" name="title" id="question_contents" value="<?php echo ($topic["title"]); ?>" class="form-control" />
									</div>
								</div>
								<h4>帖子内容:</h4>
								...
								<div class="sl-mod sl-editor-box">
									<div class="mod-head">
										<div class="wmd-panel">
								            <textarea class="wmd-input form-control autosize editor" id="content" rows="8" name="content"  placeholder="内容至少5个字符"><?php echo ($topic["content"]); ?></textarea>
								        </div>
									</div>
									<h4>选择版块:</h4>
								<div class="sl-publish-title active">
									<div>
										<select name="forum" class="form-control">
											<?php if(is_array($forum_list)): $i = 0; $__LIST__ = $forum_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$forum): $mod = ($i % 2 );++$i;?><option value="<?php echo ($forum["id"]); ?>" <?php if($forum['id'] == $topic['forum_id']): ?>selected = "selected"<?php endif; ?>><?php echo ($forum["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
									</div>
								</div>
									<div class="mod-body">
								...
								<div style="margin-top:10px;">
									<input type="submit" value="更新帖子" class="btn btn-large btn-success btn-publish-submit pull-right" style="margin-bottom: 10px;" />
								</div>
							</div>
						</div>
					</form>
...
<div id="footer">
	Copyright © 2015 - SLBBS, All Rights Reserved<br/>
	Powered By <a href="/" target="blank">SLBBS 1.0</a>
</div>
</body>
</html>