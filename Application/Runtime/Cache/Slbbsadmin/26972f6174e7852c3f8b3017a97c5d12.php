...
<div class="header">
	...
	<div class="cf">
		<div class="fl">
            ...
            <button class="btn" url="<?php echo U('Slbbsadmin/User/authManager/id/'.$id.'/value/0');?>">禁 用</button>
    ...
    <tbody>
		<?php if(!empty($user)): if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><span><?php echo ($vo["manager"]); ?></span></td>
			<td><?php echo ($vo["topic_count"]); ?></td>
			<td><?php echo ($vo["reply_count"]); ?></td>
			<td><?php if(($vo["power"]) == "0"): ?><a href="<?php echo U('Slbbsadmin/User/authManager/id/'.$vo['id'].'/value/1');?>">启用</a>
				<?php else: ?>
				<a href="<?php echo U('Slbbsadmin/User/authManager/id/'.$vo['id'].'/value/0');?>">禁用</a><?php endif; ?>
